 ...
 */function formatSession(session) {return session.split(':')[0];}const verifyToken = (req, res, next) => {const secureToken = req.serverOptions.secretKey;const { session } = req.params;const { authorization: token } = req.headers;if (!session) return res.status(401).send({ message: 'Session not informed' });try {let tokenDecrypt = '';
    let sessionDecrypt = '';
    ...
    try {
      ...
      try {
        if (token && token !== '' && token.split(' ').length > 0) {
          ...
          if (token_value) tokenDecrypt = token_value.replace(/_/g, '/').replace(/-/g, '+');else
